...
|-------------------------------------------------------*/
  grunt.initConfig({
	...
	// add to loadNpmTasks list and register
	// add settings below - proxy has to be set for each deployment

	browserSync: {
		default_options: {
			bsFiles: {
				src: [
					'<%= globalConfig.sitePath %><%= globalConfig.srcJS %>',
					'<%= globalConfig.sitePath %><%= globalConfig.watchCSS %>',
					'<%= globalConfig.sitePath %><%= globalConfig.srcInc %>*.php'
				]
			},
			options: {
				livereload: true,
