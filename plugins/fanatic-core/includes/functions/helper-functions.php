...
// Repeater for tracking codes

function renderTrackingCodes($locationPosition)
{

	if (have_rows('tracking_codes', 'option')):

		while (have_rows('tracking_codes', 'option')) : the_row();

			$location = get_sub_field('script_location');
//			print_r($location);

			if ($location == $locationPosition) {
				the_sub_field('tracking_script');
			}
